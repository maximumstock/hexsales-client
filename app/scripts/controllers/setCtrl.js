	...
	function getArticles() {
      ...
      .then(function(res) {
        ...
        $scope.articles.forEach(function(article) {
          ...
          // determine most recent keys (1-15) for both currencies and attach found
          // pricelist data to each article
          ['platinum', 'gold'].forEach(function(currency) {
            try {
              var key = Object.keys($scope.pricelist[currency][uuid])[0];
              if(key) {
                article[currency] = $scope.pricelist[currency][uuid][key];
                Api.formatAllDemNumbers(article[currency])
                article[currency].key = key;
              }
            } catch (e) {
              return;
            }
